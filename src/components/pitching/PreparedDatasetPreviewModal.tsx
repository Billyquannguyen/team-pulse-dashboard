...
import { ArrowDown, ArrowUp, ArrowUpDown, Copy, Download, Sparkles } from "lucide-react";
...
type Props<Row extends PreparedRow> = {
  open: boolean;
  ...
  onDownload: (rows: Row[], columns: PreviewColumn<Row>[]) => void;
};
      ...
      <DialogContent className="flex h-[92vh] w-[96vw] max-w-none flex-col gap-0 overflow-hidden rounded-3xl border-border bg-card p-0 sm:rounded-3xl">
        <DialogHeader className="border-b border-border px-6 py-5 pr-14">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <DialogTitle>Pitching sheet preview</DialogTitle>
              <DialogDescription className="mt-1">
                {displayedRows.length.toLocaleString()} prepared rows ·{" "}
            ...
            <div className="flex flex-wrap items-center gap-2">
              {smartSort && (
                ...
                >
                  ...
                  Smart Sorting {smartSorting ? "On" : "Off"}
                </Button>
              )}
              <Button
                type="button"
              ...
              <Button
                ...
                disabled={displayedRows.length === 0}
                onClick={() => {
                  onDownload(displayedRows, columns);
        ...
        {columns.length === 0 || displayedRows.length === 0 ? (
          <div className="flex flex-1 items-center justify-center p-8 text-center">
            <div>
              <div className="text-base font-semibold">Nothing to preview</div>
              <p className="mt-1 text-sm text-muted-foreground">
                Adjust the filters until at least one creator is prepared.
              </p>
            </div>
          </div>
        ) : (
                ...
                <tr>
                  <th className="sticky left-0 top-0 z-30 w-12 border-b border-r border-border bg-muted px-3 py-3 text-center">
                    <input
                      ...
                      aria-label={
                        allSelected ? "Clear all displayed rows" : "Select all displayed rows"
                      }
                  ...
                  {columns.map((column) => {
                    ...
                    return (
                      <th
                        key={column.key}
                        className="sticky top-0 z-20 min-w-40 border-b border-r border-border bg-muted px-3 py-3 text-left text-xs font-semibold uppercase tracking-wide text-muted-foreground"
                      >
                        <button
                          type="button"
                          onClick={() => cycleSort(column)}
                          disabled={column.sortable === false}
                          className="inline-flex w-full items-center justify-between gap-2 rounded-md text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:cursor-default"
                          aria-label={`Sort by ${column.label}`}
                        >
                          {column.label}
                          {column.sortable !== false && <SortIcon className="h-3.5 w-3.5" />}
                        </button>
                      </th>
                    );
                  })}
                    ...
                    >
                      ...
                      {columns.map((column) => {
                        ...
                        return (
                          <td
                            key={column.key}
                            className="max-w-sm whitespace-pre-wrap border-b border-r border-border px-3 py-2.5 align-top"
                          >
                            {isValidUrl(text) ? (
                              <a
                                href={text}
