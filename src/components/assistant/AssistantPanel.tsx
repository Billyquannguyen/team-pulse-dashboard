...
function mockReply(question: string): string {
  ...
  if (q.includes("script") || q.includes("outreach"))
    return "The current outreach scripts live in the Notion Handbook → Outreach → /scripts. The top performer this month is the \"warm-intro v3\" template — reply rate ~14%.";
...
export function AssistantPanel() {
  ...
  const [messages, setMessages] = useState<Msg[]>([
    { role: "assistant", content: "Hey! I'm your Team Billion AI work assistant 👋 Ask me anything about outreach, creator follow-ups, or brand objections." },
  ]);
  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-20 right-4 md:bottom-6 md:right-6 z-30 flex items-center gap-2 rounded-full bg-primary px-5 py-3 text-sm font-semibold text-primary-foreground shadow-xl transition hover:scale-105"
      >
          ...
          >
            <div className="flex items-center justify-between border-b border-border p-4">
              <div className="flex items-center gap-3">
                <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-fun-lime">
                  <Bot className="h-5 w-5" />
                </div>
                <div>
                  <div className="text-sm font-semibold">AI Work Assistant</div>
                  <div className="text-xs text-muted-foreground">Trained on the Team Billion playbook</div>
                </div>
              </div>
                ...
                <div className="space-y-2 pt-2">
                  ...
                  {SUGGESTIONS.map((s) => (
                    <button
                      key={s}
                      onClick={() => send(s)}
                      className="block w-full rounded-2xl bg-fun-yellow/60 px-4 py-2.5 text-left text-sm font-medium hover:bg-fun-yellow"
                    >
                      {s}
                    </button>
                  ))}
            ...
            >
              <input
                ...
                className="flex-1 rounded-2xl border border-border bg-background px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-primary/30"
              />
              <button type="submit" className="flex h-11 w-11 items-center justify-center rounded-2xl bg-primary text-primary-foreground hover:opacity-90">
