...
export function AssetCard({ asset }: { asset: AssetLink }) {
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className={`group relative overflow-hidden rounded-3xl bg-gradient-to-br ${asset.accent} p-6 ring-1 ring-border transition hover:-translate-y-0.5 hover:shadow-lg`}
    >
      ...
      <ExternalLink className="absolute right-5 top-5 h-4 w-4 opacity-50 transition group-hover:opacity-100" />
    </a>
  );
}
